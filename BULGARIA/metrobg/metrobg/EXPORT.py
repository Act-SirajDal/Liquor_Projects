...
def file_generation():
    ...
    path = f"D:\\BACKUP\\HINAL\\DATA\\METRO\\{month_date}\\"
    # path = f"C:\\BACKUP\\HINAL\\DATA\\METRO\\{month_date}\\"
    ...
    with pd.ExcelWriter(path + f'METRO_{month_date}.xlsx', engine='xlsxwriter') as writer:
        try:
            print(len(df))
