...
from kaufland.items import KauflandData
from kaufland.EXPORT import file_generation
...
class LinkSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = 'https://glovoapp.com/bg/bg/sofiya/kaufland-sof/?content=napitki-sc.302724563%2Fvisokoalkoholni-napitki-c.1772584864'
        # self.pagesave = 'D:/Glovoapp/'
        ...
        pl_file_path = f"{db.PAGESAVE}/product_listing.html"
        ...
        if id_lists:
            ...
            for i in id_lists:
                ...
                headers = {
                    ...
                    'glovo-device-urn': 'glv:device:633c1593-3c83-4954-a2d6-6180a58b056a',
                    'glovo-dynamic-session-id': '9d36023a-0f0e-46b7-8c12-8842e1538952',
                    'glovo-language-code': 'bg',
                    'glovo-location-city-code': 'SOF',
                    'glovo-perseus-client-id': '1715194203568.307222943685248023.mkafz76lhd',
                    'glovo-perseus-session-id': '1719307671925.847766386526629067.e864rwid8y',
                    'glovo-perseus-session-timestamp': '1719307671925',
                    'glovo-request-id': '897bdaf2-c3ff-4bf1-8f15-91c5b9ef92e7',
                ...
                if os.path.exists(pdp_path):
                    ...
                    # if id == '4611686018430391482':
                    url = f'https://api.glovoapp.com/v3/stores/195433/addresses/338048/products/{id}'
    ...
    def parse1(self, response, **kwargs):
        ...
        if Product_id:
            ...
            if match:
                years = match.group()
                item['Age_of_Whiskey'] = years .replace('Год','')
                print(years)

            # todo for ABV,image_url
            abvs = re.findall(r'([-+]?(?:\d*[\,|\.]*\d+))\s*%', SKU_Name, flags=re.IGNORECASE)
            if not abvs:
                abvs = re.findall(r'([-+]?(?:\d*[\,|\.]*\d+))\s*°', SKU_Name, flags=re.IGNORECASE)
            if abvs:
                abvs1 = abvs[0]
                item['ABV']=abvs1.replace(',','.').replace('%','')
            ...
            item['Platform_Name'] = "Kaufland.bg"
            item['Platform_URL'] = "https://glovoapp.com/bg/bg/sofiya/kaufland-sof/?content=napitki-sc.302724563%2Fvisokoalkoholni-napitki-c.1772584864"
...
if __name__ == '__main__':
    execute("scrapy crawl data_code".split())