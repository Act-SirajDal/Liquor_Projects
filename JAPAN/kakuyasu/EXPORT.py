...
def file_generation():
    ...
    path = f'D:\\BACKUP\\HINAL\\DATA\\KAKUYASU\\{db.month_date}\\'
    # path = f'C:\\BACKUP\\HINAL\\DATA\\KAKUYASU\\{db.month_date}\\'
    if not os.path.exists(path):
        os.makedirs(path)

    # Specify columns to translate
    columns_to_translate = ["Category", "SKU_Name", "Country_of_Origin"]
    ...
    with pd.ExcelWriter(path + f'Kakuyasu_{db.month_date}.xlsx', engine='xlsxwriter',) as writer:
        try:
            # del df['Category_1']
            # del df['Category_2']
            print(len(df))
