...
def file_generation():
    ...
    # path = f"C:\\BACKUP\\HINAL\\DATA\\AMAZON\\JP\\{db.month_date}\\"
    path = f"D:\\BACKUP\\HINAL\\DATA\\AMAZON\\JP\\{db.month_date}\\"
    ...
    with pd.ExcelWriter(path + f'Amazon_{db.month_date}.xlsx', engine='xlsxwriter',) as writer:
        try:
            # del df['Category_1']
            # del df['Category_2']
            print(len(df))
